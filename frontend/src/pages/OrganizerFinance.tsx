            ...
            <div className="grid grid-cols-1 gap-5 animate-slide-down" style={{ animationDelay: '100ms', animationFillMode: 'both' }}>
              <div className="bg-white p-5 rounded-[1.5rem] border border-gray-200 shadow-sm">
                <div className="flex items-center justify-between mb-8">
                  <div>
                    <h3 className="text-lg font-black text-gray-900">Biểu đồ doanh thu</h3>
                    <p className="text-xs text-gray-500 font-medium">Theo tháng trong năm (VND)</p>
                  </div>
                </div>
                  ...
                  {chartData.map((val, i) => {
                    ...
                    return (
                      <div key={i} className="flex-1 h-full flex flex-col items-center justify-end gap-3 group relative cursor-pointer">
                        {/* Tooltip */}
                        <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-[10px] font-black px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-all pointer-events-none z-10 whitespace-nowrap">
                          {formatCurrency(val)}
                        </div>
                        ...
                        <div className="w-full flex-1 bg-gray-50/80 rounded-t-xl relative overflow-hidden ring-1 ring-inset ring-gray-100 border-b-2 border-gray-200">
                        ...
                        <span className="text-[9px] text-gray-400 font-bold uppercase group-hover:text-indigo-600 transition-colors">T{i + 1}</span>
                      </div>
                    )
                  })}
                </div>
                ...
                <table className="w-full text-left whitespace-nowrap">
                  <thead>
                    <tr className="bg-gray-50/50">
                      {['Mã GD', 'Sự kiện', 'Khách hàng', 'Tổng thu', 'Phí nền tảng', 'Thực nhận', 'Ngày', 'Trạng thái'].map((h) => (
                        <th key={h} className="p-6 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {orders.length === 0 && !loading && (
                      <tr>
                        <td colSpan={8} className="p-12 text-center text-gray-400 font-bold text-sm italic">Chưa có giao dịch nào phát sinh</td>
                      </tr>
                    )}
                    {paginatedOrders.map((tx) => {
                      ...
                      return (
                        <tr key={tx.id} className="hover:bg-indigo-50/30 transition-colors group">
                          <td className="p-6 text-xs font-bold text-gray-400 font-mono">#{tx.id}</td>
                          <td className="p-6">
                            <div 
                              ...
                              <div className="w-10 h-10 rounded-lg overflow-hidden bg-gray-100 shrink-0 border border-gray-200">
                                {tx.eventPosterUrl ? (
                                  ...
                                  <div className="w-full h-full flex items-center justify-center">
                                    <Icon name="event" className="text-gray-400" size="sm" />
                                  </div>
                                )}
                              </div>
                              <span className="text-sm font-black text-gray-800 tracking-tight group-hover/event:text-primary transition-colors max-w-[200px] truncate" title={tx.eventTitle}>
                                {tx.eventTitle}
                              </span>
                            </div>
                          </td>
                          ...
                          <td className="p-6 text-xs font-bold text-gray-500 whitespace-nowrap">{new Date(tx.purchaseDate).toLocaleString('vi-VN')}</td>
                          <td className="p-6 whitespace-nowrap">
                            <span className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest inline-block border shadow-sm ${tx.status === 'COMPLETED' ? 'bg-emerald-500 text-white border-emerald-600 shadow-emerald-100' :
                                tx.status === 'PENDING' ? 'bg-orange-400 text-white border-orange-500 shadow-orange-100' :
                                  'bg-gray-400 text-white border-gray-500 shadow-gray-100'
                              }`}>{tx.status}</span>
                          </td>
                        </tr>
                      )
                ...
                <div className="p-4 bg-gray-50/50 border-t border-gray-100 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="w-8 h-8 rounded-lg flex items-center justify-center border border-gray-200 bg-white text-gray-600 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 transition-all"
                    >
